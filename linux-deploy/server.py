...
from config import ACCESS_PASSWORD, ACCESS_USER
from expiry import (
    MAX_UPLOAD_BYTES,
    MAX_UPLOAD_NOTICE,
    RETENTION_NOTICE,
...
from paths import safe_filename, safe_resolve_under_root
...
def create_app(state: TransferState) -> Flask:
    ...
    def _require_basic_auth():
        """公网部署时启用 HTTP Basic Auth；/healthz 供 systemd 探活，不鉴权。"""
        if request.path == "/healthz":
            return None
        if not ACCESS_PASSWORD:
            return None
        auth = request.authorization
        user_ok = auth is not None and secrets.compare_digest(auth.username or "", ACCESS_USER)
        pass_ok = auth is not None and secrets.compare_digest(auth.password or "", ACCESS_PASSWORD)
        if user_ok and pass_ok:
            return None
        return (
            "需要登录",
            401,
            {"WWW-Authenticate": 'Basic realm="LAN File Transfer"'},
        )
